...
CREATE TABLE IF NOT EXISTS `mydb`.`pacientes` (
  ...
  UNIQUE INDEX `credencial_paci_UNIQUE` (`credencial_paci` ASC) VISIBLE,
  CONSTRAINT `FK_cpe`
  ...
  CONSTRAINT `FK_usuario`
    FOREIGN KEY (`id_paci`)
    REFERENCES `mydb`.`usuarios` (`pacientes_id_paci`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
